...
Treat the published enums as open sets rather than closed ones. The documentation is not
self-consistent about them — the prose omits actions the schema includes — so a value outside
the published list is better handled as possible than as invalid.
...
gh secret set SNOWFLAKE_ACCOUNT     --body "<your-account-identifier>"
gh secret set SNOWFLAKE_PRIVATE_KEY < ~/.snowflake-keys/ci_key_pkcs8.pem
```

**The account identifier is a secret here, not a variable, and on a public repository that
matters.** Workflow logs are world-readable, and the workflows set these at job level, so
every step prints its whole environment block. A variable is echoed in clear text on every
run; a secret is masked. The object names above are safe to publish — they are the same
names this repository documents — but the account identifier is yours.
...
documentation; no real moderation content, accounts or personal data are included.
