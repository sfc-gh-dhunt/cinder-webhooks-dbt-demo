...
SHAPE FIDELITY
--------------
The payloads here deliberately go beyond the published examples. A published schema is a
lower bound on what a webhook can carry, so the seeds exercise the variations a consumer
has to survive rather than only the happy path:

  * `job_category` carries values outside the published four-value enum (for example
    `qa_appeal`), so enum tests in this project warn rather than fail.
  ...
  * Reviewer groups include outsourced-moderation vendor names, a common way to segment
    moderator populations.
...
# Deliberately wider than the published enum. `qa_appeal` in particular does not
...
def iso_zulu(dt: datetime) -> str:
    """The other shape to survive: Zulu, microseconds or none. Both must parse."""
...
def job_actioned_rows() -> tuple[list[tuple[str, str, str]], dict[str, int]]:
    ...
    for job in JOBS:
        ...
        for i in range(n_actions):
            ...
            job_block = {
                "id": job["id"],
                # Carried here although the published schema does not promise it.
                # Job age at action time depends on it.
            ...
            # The entity object is not guaranteed. Models must not assume it is
            # there.
            if rng.random() < 0.75:
